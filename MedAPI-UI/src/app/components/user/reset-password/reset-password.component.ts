...
export class ResetPasswordComponent implements OnInit {
  ...
  resetPassword() {
    ...
    this.patientService
      .resetPassword(this.id, encodeURIComponent(this.code), pwd)
      .then((response: any) => {
      ...
      .catch((error: any) => {
        this.toastr.error(
          '¡Algo salió mal! Póngase en contacto con el servicio de asistencia..'
        );
        this.router.navigateByUrl('/login');
      });
  }
  ...
  passwordConfirming(
    c: AbstractControl
  ): { invalid: boolean; passwordConfirming: boolean } {
    var self = this;
